...
journal entry, or `HANDOFF.md` when a session stopped mid-task.
...
| M17 Capability broker | **Done**, released as **4.0.0** on 2026-09-24. Pure core [CAP-2, CAP-5, CAP-9], the `pre_tool` veto stage [CAP-3, CAP-6], intent creation [CAP-1], `--scope`/`/scope`, `task` attenuation on delegation [CAP-9], the controller's `tighten_policy` attenuating the live ticket too (CTRL-8), the signed receipt [ADR-0039], `edgar receipt`, `[broker] enabled`, the confused-deputy acceptance test and the tour (`docs/tour/broker.html`) | 4.0 |
| M16 Scheduling | **Done**, released as **4.0.0** on 2026-09-24. `schedule/parser.py`, `due.py` pure [SCH-4, SCH-5]; `tick.py` overlap prevention and catch-up [SCH-6, SCH-7]; `install.py` for cron/launchd/Task Scheduler [SCH-3]; `schedules.toml` and `edgar schedule add` [SCH-1, SCH-12]; per-entry mode/agent/model/allowlist/verify [SCH-8]; run transcripts and the `session_end` hook [SCH-9, SCH-10, with `examples/extensions/schedule-notify/`]; `schedule_self` in its own `self_schedules` table with a pending cap, daily rate limit and depth ceiling, tagged `[self]` by `schedule list` [SCH-11]; scheduled runs and self-schedules carry broker tickets, `actor schedule:NAME`, a self-schedule's ticket attenuated from its creator's [CAP-1, CAP-4, CAP-5]; the tour (`docs/tour/schedule.html`) | 4.0 |
