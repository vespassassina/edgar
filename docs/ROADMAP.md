...
| M17 Capability broker | In progress on `feat/m17-capability-broker` — pure core [CAP-2, CAP-5, CAP-9], the `pre_tool` veto stage [CAP-3, CAP-6], intent creation [CAP-1] and `--scope`/`/scope` (live end to end) built and tested; `task` attenuation, receipt, its CLI command, config and tour are not started; 36 LOC of headroom left outside removable packages, see `docs/HANDOFF.md` | 4.0 |
