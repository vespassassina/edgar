...
| M17 Capability broker | In progress on `feat/m17-capability-broker` — pure core [CAP-2, CAP-5, CAP-9], the `pre_tool` veto stage [CAP-3, CAP-6] and intent creation [CAP-1] built and tested; receipt, `--scope`/`/scope`, CLI, config and tour are not started; 78 LOC of headroom left outside removable packages, see `docs/HANDOFF.md` | 4.0 |
