...
| PRV-17 | Prompt profiles, set with `[prompt] profile =` (`full` or `compact`, default `auto`). `compact` uses `prompts/compact.md`, exposes Core built-ins only unless configured, and lowers `compact_at` and `compact_to` by 0.1. Chosen automatically for models with `max_context` under 32k, overridable per model | Must |
...
| ROUTE-2 | Declarative routing rules, written as `[[route]]` entries, first match wins, over a fixed condition set: `role`, `agent`, `mode`, `tools_required`, `prompt_tokens_lt/_gt`, `budget_remaining_lt`, `tags`, `schedule` | Must |
